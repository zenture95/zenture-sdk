...
Low-level create methods and high-level `.run(...)` helpers both require a
caller-provided key. This prevents duplicate billable work when a create request
reaches the API but the response is lost before local polling begins.
